...
export default function StockPage() {
    ...
    try {
      ...
      if (menuItemIds.length > 0) {
        ...
        if (!itemsError && itemsData) {
          itemsMap = itemsData.reduce((acc: Record<string, { id: string; name: string; making_cost: number; category: string }>, row: unknown) => {
            const item = row as { id: string; name: string; cost?: number; category?: { name: string } | { name: string }[] | null }
            const cat = item.category
            const catName = Array.isArray(cat) ? cat[0]?.name : cat?.name
            acc[item.id] = {
              id: item.id,
              name: item.name,
              making_cost: item.cost ?? 0,
  ...
  const totalStockValue = stockData.reduce((sum: number, item: StockSummary) => sum + (item.current_value || 0), 0)
  const totalInValue = stockData.reduce((sum: number, item: StockSummary) => sum + (item.total_in_value || 0), 0)
  const totalOutValue = stockData.reduce((sum: number, item: StockSummary) => sum + (item.total_out_value || 0), 0)
    ...
    <div className="min-h-screen bg-slate-50 p-3 lg:p-6">
      <div className="mb-4 lg:mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Link href="/stock/ledger" className="px-4 py-3 bg-slate-600 text-white rounded-lg font-bold hover:bg-slate-700 text-center">
            📜 View Ledger
          </Link>
          <button onClick={() => setShowOpeningModal(true)} className="px-6 py-3 bg-blue-500 text-white rounded-lg font-bold hover:bg-blue-600">
            📝 Set Opening Balance
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 lg:gap-4 mb-6">
        <div className="bg-blue-50 rounded-xl p-4 border-2 border-blue-200">
          ...
          <div className="text-2xl lg:text-3xl font-bold text-blue-700">৳{totalStockValue.toFixed(2)}</div>
        </div>
        <div className="bg-emerald-50 rounded-xl p-4 border-2 border-emerald-200">
          <div className="text-xs text-emerald-600 mb-1">Total IN (Purchases)</div>
          <div className="text-2xl lg:text-3xl font-bold text-emerald-700">৳{totalInValue.toFixed(2)}</div>
        </div>
        <div className="bg-red-50 rounded-xl p-4 border-2 border-red-200">
          <div className="text-xs text-red-600 mb-1">Total OUT (Sales)</div>
          <div className="text-2xl lg:text-3xl font-bold text-red-700">৳{totalOutValue.toFixed(2)}</div>
        </div>
        <div className="bg-purple-50 rounded-xl p-4 border-2 border-purple-200">
          <div className="text-xs text-purple-600 mb-1">Total Items</div>
          <div className="text-2xl lg:text-3xl font-bold text-purple-700">{stockData.length}</div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handleSetOpening} className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Select Item *</label>
                <select value={selectedItem} onChange={(e) => setSelectedItem(e.target.value)} required className="w-full px-4 py-3 border-2 border-slate-200 rounded-lg text-slate-900">
                  <option value="">Choose item...</option>
                  {menuItems.map(item => (
                    <option key={item.id} value={item.id}>{item.name} (Cost: ৳{item.making_cost})</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Opening Quantity *</label>
                <input type="number" value={openingQty} onChange={(e) => setOpeningQty(e.target.value)} step="0.01" min="0" required className="w-full px-4 py-3 border-2 border-slate-200 rounded-lg text-slate-900" placeholder="e.g., 100" />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Opening Value (৳) *</label>
                <input type="number" value={openingValue} onChange={(e) => setOpeningValue(e.target.value)} step="0.01" min="0" required className="w-full px-4 py-3 border-2 border-slate-200 rounded-lg text-slate-900" placeholder="e.g., 5000.00" />
                {openingQty && openingValue && (
                  <p className="text-xs text-slate-500 mt-1">Unit cost: ৳{(parseFloat(openingValue) / parseFloat(openingQty)).toFixed(2)}</p>
                )}
              </div>
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setShowOpeningModal(false)} className="flex-1 px-4 py-3 bg-slate-200 text-slate-700 rounded-lg font-bold">Cancel</button>
                <button type="submit" className="flex-1 px-4 py-3 bg-emerald-500 text-white rounded-lg font-bold">Set Balance</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl p-12 text-center border-2 border-slate-200">
          ...
          <div className="text-slate-600 mb-4">Set opening balances or make purchases to start tracking stock values</div>
          <button onClick={() => setShowOpeningModal(true)} className="px-6 py-3 bg-blue-500 text-white rounded-lg font-bold hover:bg-blue-600">📝 Set Opening Balance</button>
        </div>
      ) : (
              ...
              <tbody>
                {stockData.map((item, idx) => (
                  <tr key={item.menu_item_id ?? idx} className="border-b hover:bg-slate-50">
                    <td className="px-4 py-3 font-bold text-slate-900">{item.menu_item?.name}</td>
                    ...
                    <td className="px-4 py-3 text-right font-semibold">{(item.current_quantity ?? 0).toFixed(2)}</td>
                    ...
                    <td className="px-4 py-3 text-right font-black text-lg text-slate-900">৳{(item.current_value ?? 0).toFixed(2)}</td>
                  </tr>
                ))}
                <tr className="border-t-2 bg-slate-100 font-bold">
                  <td colSpan={3} className="px-4 py-4 text-right">TOTAL:</td>
                  <td className="px-4 py-4 text-right text-blue-700">৳{stockData.reduce((s: number, i: StockSummary) => s + (i.opening_value ?? 0), 0).toFixed(2)}</td>
                  <td className="px-4 py-4 text-right text-emerald-700">+৳{(totalInValue ?? 0).toFixed(2)}</td>
                  <td className="px-4 py-4 text-right text-red-700">-৳{(totalOutValue ?? 0).toFixed(2)}</td>
                  <td className="px-4 py-4 text-right text-xl text-slate-900">৳{(totalStockValue ?? 0).toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
